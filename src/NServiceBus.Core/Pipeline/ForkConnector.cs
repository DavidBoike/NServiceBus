namespace NServiceBus.Pipeline
{
        ...
        where TForkContext : IBehaviorContext
    {
        ...
        public abstract Task Invoke(TFromContext context, Func<Task> next, Func<TForkContext, Task> fork, CancellationToken token);

        /// <inheritdoc />
        public sealed override Task Invoke(TFromContext context, Func<Task> next, CancellationToken token)
        {
            Guard.AgainstNull(nameof(context), context);
            Guard.AgainstNull(nameof(next), next);

            return Invoke(context, next, ctx => ctx.InvokePipeline(token), token);
        }
    }
}